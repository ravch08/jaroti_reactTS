...
const Cart = () => {
	...
	return (
		<React.Fragment>
			...
			>
				<SidebarHeader quantity={totalCartQuantity} closeSidebar={closeCartSidebar} />
				<SidebarBody
					cart={cart}
					add={(value) => dispatch(incrementQuantity(value))}
					delete={(value) => dispatch(deleteFromCart(value))}
					subtract={(value) => dispatch(decrementQuantity(value))}
				/>
				<SidebarFooter price={totalPrice} closeSidebar={closeCartSidebar} />
			</Stack>
		</React.Fragment>
	);
};
